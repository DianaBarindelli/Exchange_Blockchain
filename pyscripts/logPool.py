...
feesf = './logs/fees.txt'  #file che contiene le info sulle fees pagate dai player
...
if len(log) > 0:
	for i in log:  #ciclo su tutti i log presenti nel contractAddress(Pool)
		...
		if sign == Bought_events:  #se il topic[0] coincide con l'hash dell'evento bought
			...
			printFile(str(fees)+data_evento, feesf) #salvo su file andamento fees
			...
			fees = data_decoded[4]
			printFile(str(pay_amount)+'\t'+str(tkn_amount)+'t'+str(price)+'t'+data_evento, pay_token_price)  #salvo l'info nel file
			printFile(str(fees)+data_evento, feesf)
			...
			print('swapperPool:'+str(data_decoded[1]))
			print('PoolB:'+str(data_decoded[2]))
			print('tokenOut_amount:'+str(data_decoded[3]))
			print('tokenIn_amount:'+str(data_decoded[4]))
			...
			printFile('swapperPool:'+str(data_decoded[1]),pool)
			printFile('PoolB:'+str(data_decoded[2]),pool)
			printFile('tokenOut_amount:'+str(data_decoded[3]),pool) #e la quantità
			printFile('tokenIn_amount:'+str(data_decoded[4]),pool)
